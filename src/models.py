"""
Pydantic SchemaForms Demo - Form Models

This module contains all the form models for the demo application.
Includes models from both the main demo and comprehensive examples.
"""
...
class ContactForm(FormModel):
    ...
    email: EmailStr = FormField(
        title="Email Address", input_type="email", placeholder="your.email@example.com", icon="mail"
    )
    ...
    priority: Priority = FormField(
        Priority.MEDIUM, title="Priority", input_type="select", icon="flag"
    )
...
def handle_form_submission(form_class, form_data, success_message="Form submitted successfully!"):
    ...
    try:
        ...
        if result.is_valid:
            ...
            return {"success": True, "message": success_message, "data": result.data}
        else:
